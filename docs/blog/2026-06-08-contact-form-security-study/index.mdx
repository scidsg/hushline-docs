...
A credible version of this study cannot ask readers to trust the charts alone.
We are publishing the redacted evidence files used to produce the article:

| Artifact | What it contains | Why it matters |
| --- | --- | --- |
| [Full redacted JSON output](pathname:///data/contact-form-study-2026.json) | Metadata, summary counts, and one record for each of the 500 ranked domains | This is the canonical evidence file. It includes homepage/contact-page status, selected contact links, form metadata, script hosts, CSP checks, canary-request summaries, and scoring fields. |
| [Deduped assessed-implementation CSV](pathname:///data/contact-form-study-2026-assessed-implementations.csv) | One row per unique assessed form implementation, deduped by `formFingerprint` | This is the easiest file for checking the headline denominator of 20 and the pass/fail columns behind each chart. |
| [Tranco top-500 input CSV](pathname:///data/contact-form-study-2026-tranco-XLPJN-top500.csv) | The fixed `XLPJN` ranking snapshot used as crawl input | This lets reviewers confirm the starting population and rerun the study against the same ranked domains. |
| [Study runner](https://github.com/scidsg/hushline-docs/blob/main/docs/scripts/contact-form-study.mjs) | The non-submitting browser measurement script | This documents how pages were selected, filled with synthetic values, scored, and redacted. |

The published files intentionally do **not** include screenshots, raw request
...
The main fields behind the claims are:
...
This evidence still has limits. It supports claims about what a browser could
...
The study supports the product premise behind Hush Line’s embeddable forms, but
the comparison should be stated in measurable terms.

For sensitive intake, the most important question is not “does the page have a
...
- **0 of 20 assessed forms** passed the combined core set of protected route,
...
- The remaining Hush Line gaps are not architectural equivalents of the common
  contact-form failures. They are explicit hardening and disclosure tasks:
  adding CSP `form-action 'self'` and surfacing retention/privacy language in
  the embed.
...
A claim we should avoid until the follow-ups ship is:
...
- iframe sandboxing and `referrerpolicy="no-referrer"` are generated in
  [hushline/embeds.py](https://github.com/scidsg/hushline/blob/main/hushline/embeds.py)
- exact embed origins are normalized and reject wildcards in
  [hushline/model/username.py](https://github.com/scidsg/hushline/blob/main/hushline/model/username.py)
- the embed form, encryption badge, recipient public-key payload, and POST route
  are rendered in
...
Those gaps should be closed before turning this study into an unqualified “Hush
Line passes the full observable standard” claim. The defensible value
proposition is strong enough without that overstatement: **Hush Line provides an
